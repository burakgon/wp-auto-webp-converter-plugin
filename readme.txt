...
On every `save_post_post`, the plugin scans the post content for `<img>` references to JPG / PNG / GIF files that live under `/wp-content/uploads`. For each, it generates a sibling `.webp` with high-quality photo encoding (quality 95) and lossless PNG conversion, then rewrites `src`, `srcset`, `data-src`, `data-lazy-src`, `data-srcset`, and Gutenberg block-comment `"url"` JSON metadata in the post HTML to point at the `.webp` twin. The original file is never deleted.
...
* Filter `wp_auto_webp_quality` to adjust lossy quality (default 95).
* Filter `wp_auto_webp_lossless` to change lossless handling (enabled for PNG by default).
...
* Encodes photos at quality 95 and PNG files as lossless WebP by default.
* Preserves ICC colour profiles and uses atomic temporary-file writes.
* Adds automatic old-sidecar upgrades plus disk-safe `wp auto-webp regenerate` commands.
